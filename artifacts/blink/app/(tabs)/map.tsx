  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.mapFrame, { borderColor: colors.border, backgroundColor: colors.secondary }]}>
        ...
        {rankedMeetups.map((item, index) => <Pressable key={item.id} onPress={() => Alert.alert(item.title, `${item.location} · ${item.time}`)} style={[styles.marker, { top: 58 + (index % 3) * 65, left: 52 + (index % 2) * 120, backgroundColor: item.isLive ? colors.primary : colors.card, borderColor: item.isLive ? colors.primary : colors.border }]}><Ionicons name={item.isLive ? 'radio' : 'people'} size={15} color={item.isLive ? colors.primaryForeground : colors.accentForeground} /></Pressable>)}
        {places.slice(0, 3).map((item, index) => <Pressable key={item.id} onPress={() => Alert.alert(item.name, item.activity)} style={[styles.placeMarker, { top: 130 + index * 48, left: 190 + (index % 2) * 54, backgroundColor: colors.card, borderColor: colors.border }]}><Feather name="coffee" size={12} color={colors.foreground} /></Pressable>)}
